...
def preferences_filepath() -> str:
    config_dir = bpy.utils.user_resource("CONFIG", path="touchview", create=True)
    if config_dir:
        return path.join(config_dir, "preferences.json")
    return path.abspath(path.dirname(__file__) + "/preferences.json")
...
class TOUCHVIEW_AP_OverlaySettings(AddonPreferences):
    ...
    def draw(self, context):
        ...
        col = box.column()
        col.prop(
            self, "input_mode", text=UILayout.enum_item_description(self, "input_mode", self.input_mode), expand=True
        )
