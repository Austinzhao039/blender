...
def is_ios() -> bool:
    return sys.platform in {"ios", "darwin"}
...
class TOUCHVIEW_OT_touch_input_3d(Operator):
    ...
    def invoke(self, context, event):
        ...
        is_quadview_orthographic = (
            context.region_data.is_orthographic_side_view and context.region.alignment == "QUAD_SPLIT"
        )
        is_locked = context.region_data.lock_rotation or is_quadview_orthographic

        in_dolly_zone = dolly_wid > self.delta[0] or self.delta[0] > context.region.width - dolly_wid
        in_pan_zone = (
            math.dist(
                self.delta,
                mid_point,  # type: ignore
        ...
        touch_navigation = is_ios() or is_touch(event)
        if touch_navigation and not in_pan_zone:
            return PASSTHROUGH

        if touch_navigation:
            self.mode = "PAN"

        if self.mode == "PAN" and touch_navigation:
            self._ios_last_mouse = Vector((event.mouse_region_x, event.mouse_region_y))
    ...
    def modal(self, context, event):
        ...
        if event.type == LMOUSE and event.value == "RELEASE":
            return FINISHED

        if event.type in {RMOUSE, "ESC"}:
            return CANCEL

        if event.type not in {"MOUSEMOVE", "INBETWEEN_MOUSEMOVE", "TRACKPADPAN", "MOUSEPAN"}:
            return MODAL

        previous_mouse = self._ios_last_mouse.copy()
        current_mouse = Vector((event.mouse_region_x, event.mouse_region_y))
        delta = current_mouse - previous_mouse
        self._ios_last_mouse = current_mouse

        if delta.length_squared == 0.0:
            return MODAL

        if self.mode == "ORBIT":
            self._ios_orbit_view(context, delta)
        elif self.mode == "PAN":
            self._ios_pan_view(context, previous_mouse, current_mouse)

        return MODAL

    def _ios_orbit_view(self, context, delta):
        if delta.x > 0:
            bpy.ops.view3d.view_orbit(type="ORBITRIGHT", angle=abs(delta.x) * IOS_ORBIT_SENSITIVITY)
        elif delta.x < 0:
            bpy.ops.view3d.view_orbit(type="ORBITLEFT", angle=abs(delta.x) * IOS_ORBIT_SENSITIVITY)

        if delta.y > 0:
            bpy.ops.view3d.view_orbit(type="ORBITUP", angle=abs(delta.y) * IOS_ORBIT_SENSITIVITY)
        elif delta.y < 0:
            bpy.ops.view3d.view_orbit(type="ORBITDOWN", angle=abs(delta.y) * IOS_ORBIT_SENSITIVITY)

        context.area.tag_redraw()

    def _ios_pan_view(self, context, previous_mouse, current_mouse):
        region = context.region
        rv3d = context.region_data
        depth_location = rv3d.view_location.copy()

        previous_location = region_2d_to_location_3d(region, rv3d, previous_mouse, depth_location)
        current_location = region_2d_to_location_3d(region, rv3d, current_mouse, depth_location)

        if previous_location is None or current_location is None:
            return

        rv3d.view_location += previous_location - current_location
        context.area.tag_redraw()
